...
Page({
          ...
          if (res.code) {
            ...
            wx.request({
              ...
              success: function(data) {
                ...
                //登录成功后请求详情
                wx.request({
                  url: app.data.requestUrl + '/Placeitem/info',
                  data: {
                    item_list_string: that.data.aaa,
  ...
  share: function(options) {
    ...
    if (wx_mini_reg_status != 3) {
      ...
      var share_data = {
        token: getApp().globalData.user.token,
        scene: v_scene,
        page: 'pages/Coupon_show/Coupon_show',
        user_id: user_id
      }
  ...
  buy: function(e) {
    var that = this
    wx.setStorageSync('orderDetail', that.data.box)
    var wx_mini_reg_status = getApp().globalData.user.wx_mini_reg_status
    if (wx_mini_reg_status != 3) {
      wx.showModal({
        title: '温馨提示',
      ...
      wx.navigateTo({
        url: '../fenxiao_commit_order/fenxiao_commit_order'
      })
    }
  },
  //返回首页
  ...
  onShareAppMessage: function() {
    ...
    if (res.from === 'button') { //我自定义的分享
      return {
        title: share_title,
        path: '/pages/Coupon_show/Coupon_show?share_btn=1' + '&user_id' + user_id + '&id=' + that.data.aaa,
      }
